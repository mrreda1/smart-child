      ...
      <div className={`no-print ${activePrintReport ? 'hidden' : 'block'}`}>
        <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-6 border-b border-gray-200 pb-8">
          <div className="flex items-center gap-6">
            ...
            />
            <div>
              <h1 className="text-4xl font-black text-gray-900 tracking-tight mb-1">{childName}'s Progress</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {Object.entries(MOCK_REPORTS_DATA)
              ...
              .map(([key, data]) => {
                ...
                let ChartComponent;
                if (key === 'memory') ChartComponent = <MemoryChartVisual data={data.history} />;
                else if (key === 'reaction') ChartComponent = <ReactionChartVisual data={data.history} />;
                else if (key === 'color')
                  ChartComponent = <ColorChartVisual history={data.history} rgb={data.rgbProfile} />;
                else if (key === 'hearing') ChartComponent = <HearingChartVisual data={data.history} />;
                else ChartComponent = <MiniLineChart data={data.history} colorClass={data.textColor} />;
                  ...
                  >
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <div>
                        ...
                        >
                          {key === 'drawing' ? 'Interactions' : 'Metrics & Trends'}
                        </p>
                      </div>
                    </div>
                    <div className="bg-gray-50 rounded-xl p-5 relative flex-none mb-6">{ChartComponent}</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
          ...
          >
            ...
            <div className="flex items-center gap-4 mb-6 pr-8">
              ...
              <div>
                <h2 className="text-2xl font-black text-gray-900 leading-tight">{selectedHistoryItem.title}</h2>
                <p className="text-gray-500 font-bold text-sm">{selectedHistoryItem.date}</p>
              </div>
            </div>
            <div className="bg-blue-50 p-5 rounded-2xl mb-6 border border-blue-100 text-blue-900">
              ...
              <p className="text-sm font-medium leading-relaxed">{selectedHistoryItem.recommendation}</p>
            </div>
                ...
                <div key={idx} className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                  <div className="flex justify-between items-center mb-3">
                    <div className="font-bold text-gray-800 flex items-center gap-2">
                      {React.createElement(MOCK_REPORTS_DATA[test.testId].icon, {
                        size: 16,
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {Object.entries(test.metrics).map(([mKey, mVal], i) => (
                      ...
                      >
                        <span className="text-[10px] uppercase font-bold text-gray-400">{mKey}</span>
                        <span className="text-sm font-black text-gray-800">{mVal}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="p-6 sm:p-10 print:p-0">
                  <div className="flex flex-col sm:flex-row justify-between items-start gap-6 border-b-2 border-gray-200 pb-8 mb-8 print:flex-row">
                    <div>
                      ...
                      <h2 className="text-xl font-bold text-gray-500 mt-2">Overall Cognitive Growth</h2>
                    </div>
                    <div className="text-left sm:text-right print:text-right flex flex-col items-start sm:items-end print:items-end w-full sm:w-auto">
                      <div className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-1">Generated On</div>
                      <div className="text-lg font-bold text-gray-800 mb-4">{new Date().toLocaleDateString()}</div>
                      <div className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-1">Child Profile</div>
                      ...
                      <div className="text-lg font-bold text-gray-800">Happy 😊</div>
                    </div>
                  </div>
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-3 mb-4">
                              ...
                              <span className="font-black text-gray-900 text-lg">{data.title}</span>
                            </div>
                            <div className="bg-gray-50 rounded-xl p-4 flex-none">{ChartComponent}</div>
                  ...
                  <div className="mt-12 border-t border-gray-200 pt-6 flex justify-between items-center">
                    <p className="text-xs font-bold text-gray-400">SMARTCHILD CONFIDENTIAL REPORT</p>
                      ...
                      <>
                        <div className="flex flex-col sm:flex-row justify-between items-start gap-6 border-b-2 border-gray-200 pb-8 mb-8 print:flex-row">
                          ...
                          <div className="text-left sm:text-right print:text-right w-full sm:w-auto">
                            ...
                            <div className="text-lg font-bold text-gray-800 mb-4 sm:mb-0">{item.date}</div>
                            ...
                            return (
                              ...
                              >
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 border-b border-gray-100 pb-4 gap-2">
                                  <div className="flex items-center gap-3">
                                    <div className={`p-2 rounded-lg ${testData.color} text-white`}>
                                      <Icon size={20} />
                                    </div>
                                    <span className="font-black text-gray-900 text-lg">{test.title}</span>
                                  </div>
                                  <div className="text-2xl font-black text-gray-900">{test.score}</div>
                                </div>
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 print:grid-cols-4">
                                  {Object.entries(test.metrics).map(([mKey, mVal], i) => (
                                    <div key={i}>
                                      ...
                                      <div className="text-xl font-black text-gray-800">{mVal}</div>
                                    </div>
                                  ))}
                                </div>
                              </div>
