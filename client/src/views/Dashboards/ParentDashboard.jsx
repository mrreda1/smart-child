...
import { ProfileModal } from "@/components/dashboard/parentDashboard/modals/ProfileModal";

const ParentDashboard = () => {
  const navigate = useNavigate();
  const { profiles, setActiveChild } = useAppContext();
  ...
  return (
    <>
        ...
        {ownerProfiles.length === 0 ? (
          <div className="border-2 border-dashed border-gray-200 bg-gray-50/50 rounded-[2rem] p-12 flex flex-col items-center justify-center text-center min-h-[300px]">
            ...
            <p className="text-gray-500 font-medium max-w-sm mb-6">
              Get started by creating a profile for your child to track their
              progress and manage play time.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {ownerProfiles.map((profile) => (
              <ProfileCard
                ...
                onDelete={() => openModal("DELETE", profile)}
                onPlay={() => openModal("PLAY_WARNING", profile)}
                onViewReports={() => {
      ...
      <section>
        ...
        {linkedProfiles.length === 0 ? (
          <div className="border-2 border-dashed border-gray-200 bg-gray-50/50 rounded-[2rem] p-12 flex flex-col items-center justify-center text-center min-h-[300px]">
            ...
            <p className="text-gray-500 font-medium max-w-sm mb-6">
              Connect to a read-only profile using a unique connection code from
              another parent.
            </p>
            <button
      ...
      {/* <LinkModal isOpen={activeModal === "LINK"} onClose={closeModal} />
      <DeleteModal
      ...
      />
      <PlayWarningModal
        isOpen={activeModal === "PLAY_WARNING"}
        profile={selectedProfile}
        onClose={closeModal}
      /> */}
    </>
