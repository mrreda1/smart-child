...
const HearingGame = ({ onFinish, difficulty = 'medium' }) => {
  ...
  useEffect(() => {
    const numOptions = difficulty === 'easy' ? 3 : difficulty === 'hard' ? 8 : 6;
    ...
    return () => {
      if (audioRef.current) {
        try {
          ...
          } else if (typeof audioRef.current.pause === 'function') {
  ...
  const playCurrentSound = async () => {
    ...
    try {
      ...
      source.onended = () => {
        ...
        setAudioEndTime((prev) => (prev === null ? Date.now() : prev));
      };
    } catch (e) {
      setIsPlaying(false);
      setHasPlayed(true);
  ...
  const handleOptionClick = (itemId) => {
    ...
    const newCorrect = isCorrect ? correctIdentifications + 1 : correctIdentifications;
      ...
      setTimeout(() => {
        onFinish(newCorrect * 10, {
          ...
          rawData: {
            ...
            correct_identifications: newCorrect,
            sum_latency_ms: newSumLatency,
          },
        });
      }, 500);
    } else {
  ...
  const roundData = rounds[currentRound];
  const gridColsClass =
    roundData.options.length > 6 ? 'grid-cols-4' : roundData.options.length > 4 ? 'grid-cols-3' : 'grid-cols-2';

  const liveISR = totalSoundsPlayed === 0 ? '0.0' : ((correctIdentifications / totalSoundsPlayed) * 100).toFixed(1);
  const liveAARL = totalSoundsPlayed === 0 ? '0.00' : (sumLatency / totalSoundsPlayed / 1000).toFixed(2);
        ...
        <div className="absolute top-[-30px] right-0 z-50 flex flex-col items-end transform translate-x-4 md:translate-x-12">
          ...
          {showDevMetrics && (
            <div className="bg-gray-900 text-green-400 font-mono text-xs p-4 rounded-2xl shadow-xl border border-gray-700 w-56 text-left">
              <div className="font-bold text-white mb-2 border-b border-gray-700 pb-2">Live Audio Metrics</div>
              <div className="mb-1 text-blue-300">Metric A: Success Rate</div>
              <div className="mb-1 ml-2">- Correct: {correctIdentifications}</div>
              <div className="mb-1 ml-2">- Sounds Played: {totalSoundsPlayed}</div>
              <div className="mb-2">
                ISR: <span className="text-white font-bold">{liveISR}%</span>
              </div>
              <div className="pt-2 border-t border-gray-700 text-blue-300 mb-1">Metric B: Latency</div>
              <div>
